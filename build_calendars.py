...
<style>
  /* Brand-ish palette: warm beige bg, deep maroon header, orange accents */
  :root {{
    --brand-maroon: #4B0D1F;
    --brand-beige:  #F3EFE6;
    --brand-orange: #FF6A00;
    ...
    --btn-apple:    #555555;
    --btn-google:   #EA4335;
    --btn-outlook:  #0F6CBD; /* Microsoft 365 blue */
  }}
  ...
  header h1 {{ margin: 0; font-size: 1.75rem; letter-spacing: .3px; }}
  ...
  label {{ font-weight: 700; display: block; margin-bottom: .5rem; }}
  select {{
    font-size: 1rem;
    padding: .6rem .8rem;
    border: 1px solid var(--border);
  ...
  select:focus {{ box-shadow: 0 0 0 3px rgba(255,106,0,.22); border-color: var(--brand-orange); }}
  ...
  .apple   {{ background: var(--btn-apple);   color: #fff; }}
  .google  {{ background: var(--btn-google);  color: #fff; }}
  .outlook {{ background: var(--btn-outlook); color: #fff; }}
...
<script>
  // Feed list generated by Python:
  const FEEDS = {feeds};

  function baseUrl() {{
    // Ensure relative links work whether hosted at / or /<repo>/public/
  ...
  // Populate dropdown (no template-literals to avoid Python f-string conflicts)
  ...
  function updateLinks() {{
    ...
    const base = baseUrl();
    const https = new URL(file, base).toString(); // final .ics https URL

    // Apple / macOS / iOS / Outlook desktop (Mac) use webcal://
    const webcal = 'webcal://' + https.replace(/^https?:\\/\\//, '');

    // Outlook WORK/SCHOOL (Microsoft 365) deep-link to "Add from web"
    // (personal outlook.com flow intentionally not used)
    ...
    // Google: copy URL and open the "Add by URL" page
